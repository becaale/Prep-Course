 ...
 */
  ...
  arrfrase.forEach((element) => {
    frainv.push(element.split("").reverse().join(""));
  });
...
function capicua(numero) {
  ...
  var txtnumero = numero.toString();
  var txtnumeroinvertido = txtnumero.split("").reverse().join("");

  if (txtnumero === txtnumeroinvertido) {
    return "Es capicua";
  } else {
    return "No es capicua";
  }
...
function sortArray(arr) {
  ...
  for (let x = 0; x <= maximo; x++) {
    for (const elemento of arr) {
      if (maximo < elemento.length) {
        maximo = elemento.length;
      }
      if (elemento.length === x) {
        arrordenado.push(elemento);
      }
    }
  }
  return arrordenado;
